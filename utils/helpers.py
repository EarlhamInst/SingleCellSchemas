...
DATA_END_ROW = 1005
DATA_START_ROW = 5
SHEET_INSTRUCTION_ROW = 4
SHEET_HEADER_ROW = 1
TERM_EXAMPLE_ROW = 2  # Row 3 becomes row 2 as index starts at 0
TERM_DESCRIPTION_ROW = 1  # Row 2 becomes row 1 as index starts at 0
TERM_SERIES_KEY = 'term_label'
...
def apply_data_validation(
    ...
    term_series = component_df[TERM_SERIES_KEY]
    column_names = term_series.drop_duplicates().tolist()
    ...
    for column_name in column_names:
        term_name = component_df.loc[term_series == column_name, 'term_name'].iloc[0]
        dropdown_list = allowed_values_dict.get(term_name, [])
        regex = (
            component_df.loc[term_series == column_name, 'term_regex'].iloc[0]
            if 'term_regex' in component_df
            else ''
        )
        term_error_message = (
            component_df.loc[term_series == column_name, 'term_error_message'].iloc[0]
        ...
        row_start, row_end = DATA_START_ROW, max(
            DATA_END_ROW, len(dataframe) + DATA_START_ROW
        )
...
def format_and_protect_worksheet(element):
    ...
    protected_columns = element['protected_columns']
    ...
    protected_format = element['protected_format']
    ...
    for col, column_name in enumerate(column_names):
        ...
        # Required/optional columns format
        if column_name in required_columns:
            # Bold the required headers
            worksheet.write(0, col, column_name, required_format)
        else:
            # Add (optional) to non-required headers
            worksheet.write(0, col, f'{column_name} (optional)', locked_format)
        ...
        # Row 2
        worksheet.write(
            TERM_DESCRIPTION_ROW,
            col,
            col_desc_eg[column_name]['description'],
            desc_eg_format,
        )
        ...
        worksheet.write(
            TERM_EXAMPLE_ROW,
            col,
            f'e.g. {col_desc_eg[column_name]["example"]}',
            desc_eg_format,
        )

        # -- Protected auto-filled columns --
        '''
        - NB: Usually, these cells would be locked but
          they are unlocked for the website
        - Apply protected format to columns related to the
          term in 'referenced_component' and 'identifier'
           columns in all worksheets except the 'study' worksheet
        - NB: 'study_id' is the term name while 'Study ID' is the term label
        '''
        if column_name in protected_columns and not (
            column_name == 'Study ID' and sheet_name == 'study'
        ):
            column_letter = get_column_letter(col + 1)

            # workbook.define_name(
            #     'StudyIDs', f'study!$A${DATA_START_ROW}:$A${DATA_END_ROW}'
            # )

            # worksheet.data_validation(
            #     f'{column_letter}{DATA_START_ROW}:{column_letter}{DATA_END_ROW}',
            #     {
            #         'validate': 'list',
            #         'source': '=StudyIDs',
            #         'input_message': 'Select a Study ID from the list',
            #     }
            # )
            for row in range(DATA_START_ROW, DATA_END_ROW + 1):
                worksheet.write_blank(f'{column_letter}{row}', None, protected_format)

    # Merge and write instruction in row 4
    merge_row(worksheet, SHEET_INSTRUCTION_ROW, last_column_letter, merge_format)

    # Set the conditional format for locking rows 1 to 4
    worksheet.conditional_format(
        f'A{SHEET_HEADER_ROW}:{last_column_letter}{SHEET_INSTRUCTION_ROW}',
        {'type': 'no_errors', 'format': locked_format},
    )

    # Set all rows below row 4 to unlocked
    worksheet.set_column(
        f'A{DATA_START_ROW}:{last_column_letter}{DATA_END_ROW}', None, unlocked_format
    )
...
def get_base_schema_json(element):
    ...
    for _, row in data_df.iterrows():
        ...
        for column in data_df.columns:
            ...
            # Explicitly convert boolean values to boolean type to avoid JSON serialisation issues with '0.0' and '1.0'
...
def get_col_desc_eg(component_df, version_column_name):
    ...
    # Use what is set as TERM_SERIES_KEY as the key
    return {
        row[TERM_SERIES_KEY]: {
...
def get_protected_columns(component_df, version_column_name):
    # NB: Usually, these cells would be locked but they are unlocked for the website
    # Use what is set as TERM_SERIES_KEY as the key
    identifier_columns = component_df.loc[
        component_df['identifier'] & component_df[version_column_name].isin(['M', 'O']),
        TERM_SERIES_KEY,
    ].tolist()

    referenced_columns = component_df.loc[
        component_df['referenced_component'].notna()
        & (component_df['referenced_component'] != '')
        & component_df[version_column_name].isin(['M', 'O']),
        TERM_SERIES_KEY,
    ].tolist()

    return list(set(identifier_columns + referenced_columns))
...
def get_required_columns(component_df, version_column_name):
    ...
    return component_df.loc[
        (component_df[version_column_name] == 'M'), TERM_SERIES_KEY
    ].tolist()
...
def load_worksheet(
    ...
    if required_columns is not None:
        if not required_columns.issubset(df.columns):
            raise ValueError(
                f"'{sheet_name}' worksheet must contain {required_columns} columns."
            )

    return df


def build_key_value_map(df, key_col='key', value_col='name'):
    ...
    try:
        ...
        if start_cell != end_cell:  # Ensure it's a valid range
            worksheet.merge_range(
                ...
                'FILL OUT INFORMATION BELOW THIS LINE',
                merge_format,
            )
        else:
            print(
...
def read_xlsx_data(return_dict=True):
    ...
    try:
        ...
        components = build_key_value_map(components_df)
        ...
        regex_to_formula = build_key_value_map(
            regex_to_formula_df,
            key_col='term_regex',
            value_col='spreadsheet_formula',
        )
...
def get_label(key, mapping, label_type):
    if key not in mapping:
        raise ValueError(f'Unknown {label_type} key: {key}')
    # Remove bracketed content such as '(scRNA-Seq)', '(FAANG)' or '[something]'
    label = re.sub(r'\s*[\(\[].*?[\)\]]', '', mapping[key]).strip()
    return label
