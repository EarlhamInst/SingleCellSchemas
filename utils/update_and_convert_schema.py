...
def generate_base_schema_yaml(file_path=None):
    '''
    ...
    data_df, allowed_values_dict = helpers.read_xlsx_data(file_path=file_path)
    ...
    if combined_yaml_data['classes']:
        ...
        output_type = helpers.REVERSE_SCHEMA_MAP.get(Path(file_name).stem)
        output_file = f'{helpers.SCHEMA_DIR_PATH}/{output_type}/{file_name}'
...
def generate_base_schema_json(file_path=None):
    '''
...
if __name__ == '__main__':
    ...
    for path in helpers.SCHEMA_FILE_PATHS:
        generate_base_schema_json(file_path=path)
        print('\n_______\n')
        generate_base_schema_yaml(file_path=path)
