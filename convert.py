...
def extract_components_to_xlsx(element):
    ...
    with pd.ExcelWriter(bytesIO, engine='xlsxwriter', mode='w') as writer:
        ...
        for component_name in component_name_series.unique():
            ...
            helpers.apply_data_validation(
                component_df, df, writer, standard_name, allowed_values_dict
            )
...
if __name__ == '__main__':
    ...
    if len(args) not in [1, 2]:
        ...
        print(
            " 3. python convert.py html: Extract components using 'html' format. Other formats that can be used are - 'xlsx', 'json, 'xml'"
        )
        sys.exit(1)
    elif len(args) == 1:
